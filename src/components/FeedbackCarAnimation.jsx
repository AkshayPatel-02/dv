...
// ─────────────────────────────────────────────────────────────────────────────
// FEEDBACK CAR ANIMATION v4
// - Base speed 600 — already fast at idle
// - Tap car to STACK speed: BOOST → SUPER → ULTRA → GODMODE (up to 3400)
// - Boost decays smoothly when tapping stops → back to base
// - Buildings BLAST backward at super speed (parallax ratio scales with speed)
// - At GODMODE: motion-blur smear on buildings, violent shake, crazy speed lines
// - Idle car always micro-shakes (alive, not static)
// - Road nitro bottles spawn + collect → add tap stack
// - NO bumps / speedbreaker — gone entirely
// ─────────────────────────────────────────────────────────────────────────────
...
export default function FeedbackCarAnimation() {
  ...
  useEffect(() => {
    ...
    const TAP_ADD       = 380;    // each tap adds this to boost
    const TAP_MAX_BOOST = 2800;   // cap — BASE + 2800 = 3400 GODMODE
    const BOOST_DECAY   = 950;    // drain rate when not tapping (per sec)
    const ACCEL         = 3500;   // speed lerp up
    const DECEL         = 650;    // speed lerp down (gradual)

    let speed       = BASE_SPEED;
    let tapBoost    = 0;
    let lastTapTime = -999;

    // ── Nitro state (visual, not primary speed source) ──
    ...
    function drawBuildings(groundY) {
      ...
      for (const b of buildings) {
        ...
        if (sx > canvas.width + 130) continue;
    ...
    function drawCar(cx, groundY, s, ang, isNitro, shX, shY, spdRatio) {
      ...
      // Update hitbox (centre-based, not shake-shifted for accurate clicking)
      carHitbox = { x1: cx-W/2-10, y1: cTop-20*s, x2: cx+W/2+12*s, y2: groundY+5 };

      // Lean forward slightly at high speed
      ...
      ctx.beginPath(); ctx.moveTo(Math.round(cabinL), Math.round(bTop+2)); ctx.lineTo(Math.round(cabinL), Math.round(bBot-3)); ctx.stroke();
    ...
    function drawFlag(poleX, poleTopY, poleBotY, s, t, spdRatio) {
      ...
      ctx.beginPath(); ctx.moveTo(pts[0][0].x+5, pts[0][0].y+7);
      for (let i=1;i<=SEG;i++) ctx.lineTo(pts[i][0].x+5, pts[i][0].y+7);
      for (let i=SEG;i>=0;i--) ctx.lineTo(pts[i][1].x+5, pts[i][1].y+7);
      ctx.closePath(); ctx.fillStyle="#000"; ctx.fill(); ctx.restore();

      ctx.beginPath(); ctx.moveTo(pts[0][0].x, pts[0][0].y);
      for (let i=1;i<=SEG;i++) ctx.lineTo(pts[i][0].x, pts[i][0].y);
      for (let i=SEG;i>=0;i--) ctx.lineTo(pts[i][1].x, pts[i][1].y);
      ...
      for (let i=1;i<=SEG;i++) ctx.lineTo(pts[i][0].x, pts[i][0].y);
      for (let i=SEG;i>=0;i--) {
        const {x:tx,y:ty}=pts[i][0], {x:bx,y:by}=pts[i][1];
      ...
      for (let c=0;c<TEXT.length;c++) {
        ...
        const tx=pts[si][0].x+(pts[sn][0].x-pts[si][0].x)*sf;
        const ty=pts[si][0].y+(pts[sn][0].y-pts[si][0].y)*sf;
        const bx_=pts[si][1].x+(pts[sn][1].x-pts[si][1].x)*sf;
        const by_=pts[si][1].y+(pts[sn][1].y-pts[si][1].y)*sf;
        ...
        const ntx=pts[nsi][0].x+(pts[nsn][0].x-pts[nsi][0].x)*nsf;
        const nty=pts[nsi][0].y+(pts[nsn][0].y-pts[nsi][0].y)*nsf;
        ctx.save();
    ...
    function loop(ts) {
      ...
      const groundY = Math.round(CH*0.725);
      const s       = Math.max(0.50, Math.min(CW/460, 1.38));
      const carCX   = CW*0.58;
      const carFront = carCX + 148*s/2;

      // ── Tap boost decay ──
      const timeSinceTap = totalTime - lastTapTime;
      if (timeSinceTap > 0.18) {
        tapBoost = Math.max(tapBoost - BOOST_DECAY*dt, 0);
      }
      if (tapBoost <= 0 && nitroLeft <= 0) nitroActive = false;

      // ── Auto nitro (just visual / tiny extra) ──
      ...
      if (nitroActive) {
        ...
        nitroExtra = 180;
        if (Math.random() < 0.50)
          spawnParticles(carCX-72*s, groundY-28*s, 1, false);
      } else {
        nitroPulse = Math.max(nitroPulse-dt*3.5, 0);
        nitroExtra = 0;
      }

      // ── Speed target = base + tap stack + nitro extra ──
      const targetSpeed_ = BASE_SPEED + tapBoost + nitroExtra;
      if (speed < targetSpeed_) {
        speed = Math.min(speed + ACCEL*dt, targetSpeed_);
      } else {
        speed = Math.max(speed - DECEL*dt, targetSpeed_);
      }

      const spdRatio = speed / BASE_SPEED;   // 1.0 at base, 5.7 at GODMODE

      // ── Scrolls — buildings parallax ratio rises steeply with speed ──
      ...
      // ── Idle + speed shake (amplitude grows quadratically) ──
      ...
      for (const p of particles) {
        p.x += p.vx*dt; p.y += p.vy*dt; p.vy += 95*dt; p.life -= dt;
      }
      ...
      // Grid — vibrates at extreme speed
      ctx.save();
      ...
      // Buildings — ghost smear at GODMODE speed (motion blur illusion)
      ctx.save();
      if (spdRatio > 2.8) {
        const smear = Math.min((spdRatio-2.8)*20, 55);
        ctx.save(); ctx.globalAlpha = 0.16;
        ctx.translate(smear, 0); drawBuildings(groundY); ctx.restore();
        ctx.save(); ctx.globalAlpha = 0.08;
        ctx.translate(smear*1.8, 0); drawBuildings(groundY); ctx.restore();
      }
      ctx.globalAlpha = 0.58;
      drawBuildings(groundY);
      ctx.restore();

      // Speed lines — cascade with speed
      ...
      const hudSz    = Math.round(Math.max(11, Math.min(17, CW*0.025)));
      ctx.font = `bold ${hudSz}px "Courier New", monospace`;
      ctx.textBaseline = "top"; ctx.textAlign = "right";
      ctx.save(); ctx.globalAlpha=0.3; ctx.fillStyle="#22c55e"; ctx.fillText(`HI ${hiStr}`,CW-16,14); ctx.restore();
      ctx.fillStyle="#22c55e"; ctx.fillText(scoreStr,CW-16,14+hudSz+4);
      ...
      if (tier.label && (tapBoost > 0 || nitroActive)) {
        ...
        ctx.font = `bold ${Math.round(hudSz*0.9)}px "Courier New", monospace`;
        ctx.textAlign="left"; ctx.textBaseline="top";
        ctx.fillStyle = tier.color;
        ctx.fillText(tier.label, 14, 14);
        ctx.restore();
      }

      // Tap boost bar (bottom-left)
      const barW = Math.round(Math.min(CW*0.18, 100));
      const barX = 14, barY = CH-22;
      ...
      if (fillR > 0) {
        ...
        ctx.fillStyle = tier.color;
        ctx.fillRect(barX, barY, Math.round(barW*fillR), 5);
      }
      ctx.restore();

      // Helper text
      const helpSz = Math.round(Math.max(8, Math.min(11, CW*0.019)));
      const helpA  = tapBoost > 100 ? 0.2 : 0.55 + Math.sin(totalTime*2.4)*0.32;
      ctx.save();
      ctx.globalAlpha = helpA; ctx.fillStyle="#22c55e";
      ctx.font = `bold ${helpSz}px "Courier New", monospace`;
      ctx.textAlign="center"; ctx.textBaseline="bottom";
      ctx.shadowColor="#22c55e"; ctx.shadowBlur=5;
      ctx.fillText("[ TAP CAR TO GO FASTER! ]", CW/2, CH-4);
      ctx.restore();
